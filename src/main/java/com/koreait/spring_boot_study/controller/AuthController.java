...
public class AuthController {
    ...
    //안에서 사용하는 변수명과 쿼리스트링의 키값이 다른경우 괄호 안에 표기해주면 됨
    //그리고 기본값도 설정이 가능함
    //그리고 다른 타입도 가능하며 여러개의 RequestParam도 받을 수 있다
    //int는 null을 허용안하기 때문에 값이 없음의 상태
    //그래서 required = false 를 했지만 에러가 뜸 => Integer로 해야 null로 받을 수 있다
    //만약 필수값이 false 이고 기본값이 설정되어있다면 필수값 설정이 무의미
...
//    @RequestBody
//    http 요청의 바디에 들어잇는 json 데이터를 자바 객체(DTO)로 변환해서 주입해주는 어노테이션
//    클리언트가 json 형식으로 데이터 보냄
//    백엔드 서버는 그 json를 @RequestBody가 붙은 DTO로 자동 매핑
//    일반적으로 POST,PUT,PATCH에서 사용
//    DTO(Data Transfer object)
//    데이터를 전달하기 위한 객체
//    클라이언트간에 데이터를 주고 받을때 사용하는 중간 객체
...
//    Post요청 singin 로그인 로직
//    SinginReqDto => email, password
